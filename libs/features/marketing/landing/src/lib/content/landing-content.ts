...
/** Re-exported so existing landing imports keep one door. */
export type {
  BarberVm,
  Localized,
  ServiceOfferingVm,
...
/**
 * Editorial extras the catalog deliberately does not carry, keyed by the
 * CATALOG's ids (`svc-*`, not the legacy fixture slugs above) and merged
 * onto whatever `CatalogReader` returns.
 *
 * This is the honest home for it: a barber's selected-work gallery and
 * one-line craft summary are marketing copy, not business data, and
 * putting them in Firestore would make every tenant author them before
 * their catalog could render. There is no `rating` here on purpose —
 * `Barber` refuses to model one, and inventing it at this layer would be
 * the same fabrication one storey up.
 */
